...
class ChanceConstraintEncoder:
    ...
    def __encode_with_saa(self):
        ...
        # Add constraints.
        for i in range(self.K):
            self.model.addCons(self.f(self.x, self.training_ys[i]) <= config.CPP.M * (1 - zs[i]))
            self.model.addCons(self.f(self.x, self.training_ys[i]) >= config.CPP.zeta + (config.CPP.m - config.CPP.zeta) * zs[i])
    ...
    def __encode_with_cpp_mip(self):
        ...
        for i in range(self.K):
            if callable(self.f):
                ...
                self.model.addCons(self.f[i] <= config.CPP.M * (1 - zs[i]))
                self.model.addCons(self.f[i] >= config.CPP.zeta + (config.CPP.m - config.CPP.zeta) * zs[i])
    ...
    def __encode_for_max(self):
        ...
        for i in range(self.K):
            ...
            for j in range(len(self.fs)):
                ...
                self.model.addCons(self.fs[j](self.x, self.training_ys[i]) - (1 - sigma[i, j]) * config.CPP.M <= mu[i])
                self.model.addCons(mu[i] <= self.fs[j](self.x, self.training_ys[i]) + (1 - sigma[i, j]) * config.CPP.M)
