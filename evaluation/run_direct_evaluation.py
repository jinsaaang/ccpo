# ccpo/evaluation/run_direct_evaluation.py
...
def run_direct_evaluation(
    ...
    try:
        ...
        temp_loader.load_data() # Load raw data to get column names
        asset_names = temp_loader.raw_data.columns.tolist()
        n_assets = len(asset_names)
        del temp_loader # Remove temporary loader
        ...
        del temp_loader_for_cpp, res_for_cpp # Clean up
        ...
        ccpo_methods = ["CCPO-CCO"] # Assuming only CCO for now
        ...
        for cpp_method in cpp_methods:
            cpp_res = run_cpp_direct(
                K_returns=K_returns_raw, # Use raw K returns for optimization
                L_returns=None,          # No L period in this setup
                V_returns=V_returns_raw, # Use raw V returns for OOS coverage check
                method=cpp_method,
                alpha=alpha,
            )
            ...
            if cpp_res.get("status") == "optimal":
                ...
                for date, asset_ret in zip(V_dates, V_returns_raw):
                    portfolios[cpp_method].add_period(
                        ...
                        solve_time=cpp_res.get("solve_time", 0.0) / len(V_dates) if len(V_dates)>0 else 0.0, # Average solve time
                        threshold_post=threshold,
                    )

        ccpo_res = run_ccpo_direct(
            data_path=cfg.DATA_PATH, # Pass path, loader will be created inside
            lookback=lookback,
            alpha=alpha,
            cfg=cfg
        )
             ...
             for pinfo in ccpo_res["portfolios"]:
                 ...
                 try:
                      ...
                      portfolios["CCPO-CCO"].add_period(
                           ...
                           solve_time=ccpo_res.get("optimization_time", 0.0) / len(V_dates) if len(V_dates)>0 else 0.0, # Avg opt time
                           threshold_post=threshold,
                      )
                 except KeyError:
