# ccpo/evaluation/run_rolling_evaluation.py
...
def run_rolling_evaluation(
    ...
    """
    Rolling window evaluation (Sliding or Expanding) based on User Diagram.
    ...
    try:
        ...
        if cfg.MODE == "counts":
            ...
            k_raw_len = cfg_roll_cnt.K_LEN # K sequences need K raw points after train_raw_len
            v_raw_len = cfg_roll_cnt.V_LEN # V sequences need V raw points after k_raw_len

            total_window_raw_len = train_raw_len + k_raw_len + v_raw_len
            step_size = cfg_roll_cnt.STEP_SIZE # Step size in terms of raw data points
            ...
            while True:
                ...
                # Check minimum length requirement for the first window
                if train_end_idx <= train_start_idx or k_end_idx <= train_end_idx or v_end_idx <= k_end_idx:
                     ...
                     current_raw_start_idx += step_size # Still advance
                     continue


                window_definitions.append({
                    "mode": "counts",
                    "train_start_idx": train_start_idx, # Raw start index for Train period data
                    "train_len": cfg_roll_cnt.MODEL_TRAIN_LEN, # Num sequences for model train
                    "K_len": cfg_roll_cnt.K_LEN,             # Num sequences for model valid (K)
                    "V_len": cfg_roll_cnt.V_LEN,             # Num sequences for test (V)
                    # For slicing raw data for CPP and getting V_returns
                ...
                if cfg_roll.WINDOW_TYPE == "sliding":
                    current_raw_start_idx += step_size
                elif cfg_roll.WINDOW_TYPE == "expanding":
                    current_raw_start_idx += step_size
                else:
            ...
            # Determine the overall start and end dates for rolling
            ...
            while True:
                ...
                if cfg_roll.WINDOW_TYPE == "expanding":
                    train_start_date = first_possible_train_start # Fixed start
                elif cfg_roll.WINDOW_TYPE == "sliding":
                    train_start_date = train_end_date - train_offset # Maintain fixed Train duration
                else:
                    raise ValueError(f"Unknown ROLLING.WINDOW_TYPE: {cfg_roll.WINDOW_TYPE}")
                ...
                window_definitions.append({
                     ...
                     "k_start_date": train_end_date, # K starts after train ends
                     "k_end_date": k_end_date,
                     "v_start_date": k_end_date, # V starts after K ends
        ...
        for i, window in enumerate(window_definitions):
            ...
            # 4.1) Extract data for K and V periods (RAW data for CPP and final eval)
            if window["mode"] == "counts":
                 ...
                 # Use .loc slicing (inclusive start, exclusive end for consistency)
            ...
            V_dates = V_data_raw.index # Dates for the final test period
            ...
            # 4.2) CPP 실행 (K 기간 원본 데이터 사용)
            window_results = {} # Store results for this window before adding to portfolio
            for cpp_method in cpp_methods:
                cpp_res = run_cpp_direct(
                    K_returns=K_returns_raw, # Optimize on K raw data
                    L_returns=None,          # No L period
                    V_returns=V_returns_raw, # Check coverage on V raw data
                    method=cpp_method,
                    alpha=alpha,
                )
                window_results[cpp_method] = cpp_res # Store result

            # 4.3) CCPO 실행 (Train/K/V 기간 정보 전달)
            if window["mode"] == "counts":
                ccpo_res = run_ccpo_rolling_counts(
                    ...
                    K_len=window["K_len"],
                    V_len=window["V_len"],
                    start_idx=window["train_start_idx"], # Raw start for the whole Train+K+V
                    V_dates=V_dates,
            ...
            window_results["CCPO-CCO"] = ccpo_res # Store result

            # 4.4) Equal-Weight 계산 (V 기간 데이터 사용)
            print("  Running Equal-Weight...")
            if n_assets > 0:
                equal_w = np.ones(n_assets) / n_assets
                # Store weights and threshold (None) for consistency
                window_results["Equal-Weight"] = {
                    'weights': equal_w,
                    'threshold_post': None,
                    'status': 'optimal', # Assume always optimal
                    'solve_time': 0.0
                }
                print(f"    ✅ Completed.")
            else:
            ...
            for method in all_methods:
                 ...
                 if result and result.get('status') == 'optimal':
                      ...
                      # Use threshold_post for CPP/Target, threshold for CCO
                      threshold = result.get('threshold_post', result.get('threshold'))
                      solve_time = result.get('solve_time', 0.0) # Get solve time if available

                      if weights is not None:
                           # Apply the *single* weight vector calculated for this window
                           # across *all* days in the V period
                           for date, asset_ret_raw in zip(V_dates, V_returns_raw):
                                portfolios[method].add_period(
                                     ...
                                     # Distribute solve time across V periods? Or store once?
                                     # Storing average for now.
        ...
        aggregate_and_save_results(
            ...
            cfg=cfg, results=None # No single summary dict for rolling
        )
